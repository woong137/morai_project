    ...
    def checkObject(self, ref_path, object_data):
        ...
        for obstacle in object_data.obstacle_list:
            for path in ref_path.poses:
                dis = sqrt(
                    pow(path.pose.position.x - obstacle.position.x, 2)
                    + pow(path.pose.position.y - obstacle.position.y, 2)
                )
                if (
                    dis < 2.35
                ):  # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
                    is_crash = True
                    break
    ...
    def latticePlanner(self, ref_path, vehicle_status):
        ...
        if len(ref_path.poses) > look_distance:
            ...
            det_trans_matrix = np.array(
                [
                    [
                        ...
                        trans_matrix[1][0],
                        -(
                            trans_matrix[0][0] * translation[0]
                            + trans_matrix[1][0] * translation[1]
                        ),
                    ],
                    [
                        trans_matrix[0][1],
                        trans_matrix[1][1],
                        -(
                            trans_matrix[0][1] * translation[0]
                            + trans_matrix[1][1] * translation[1]
                        ),
                    ],
                    [0, 0, 1],
                ]
            ...
            world_ego_vehicle_position = np.array(
                [[vehicle_pose_x], [vehicle_pose_y], [1]]
            )
            local_ego_vehicle_position = det_trans_matrix.dot(
            ...
            for i in range(len(out_path)):
                globals()["lattice_pub_{}".format(i + 1)] = rospy.Publisher(
                    "/lattice_path_{}".format(i + 1), Path, queue_size=1
                )

...
class MGeoPlannerMap(object):  # super method의 argument로 전달되려면 object를 상속해야함 (Python2에서)
    ...
    def set_coordinate_system_from_prj_file(self, prj_file):
        ...
        """
        self.global_coordinate_system = MGeoPlannerMap.esri_prj_to_proj4_string(
            prj_file)
    ...
    def to_json(self, output_path):
        ...
        if self.synced_light_set is not None:
            MGeoPlannerMap.save_synced_traffic_light(
                output_path, self.synced_light_set)

        if self.intersection_controller_set is not None:
            MGeoPlannerMap.save_intersection_controller(
                output_path, self.intersection_controller_set)
        ...
        if self.lane_marking_set is not None:
            MGeoPlannerMap.save_lane_marking(
                output_path, self.lane_marking_set)
    ...
    def save_intersection_controller(output_path, intersection_controller_set):
        ...
        file_path = os.path.join(
            output_path, 'intersection_controller_set.json')
        with open(file_path, 'w') as f:
            json.dump(save_info_list, f, indent=2)
    ...
    def load_node_and_link(folder_path):
        ...
        if not os.path.isfile(filename):
            Logger.log_warning(
                'There is no global_info.json file in the specified location. link format ver1 is assumed.')

            from save_load import subproc_load_link_ver1
            node_set, link_set = subproc_load_link_ver1.load_node_and_link(
                node_save_info_list, line_save_info_list)
    ...
    def load_traffic_sign(folder_path, link_set):
        ...
        if os.path.isfile(filename):
            ...
            Logger.log_warning(
                'There is no traffic_sign_set.json. An empty SignalSet instance will be returned.')
            return ts_set
    ...
    def load_traffic_light(folder_path, link_set):
        ...
        if os.path.isfile(filename):
            ...
            Logger.log_warning(
                'There is no traffic_light_set.json. An empty SignalSet instance will be returned.')
            return tl_set
    ...
    def load_synced_traffic_light(folder_path, link_set, tl_set):
        ...
        if os.path.isfile(filename):
            ...
            Logger.log_warning(
                'There is no synced_traffic_light_set.json. An empty SyncedSignalSet instance will be returned.')
    ...
    def load_intersection_controller(folder_path, synced_tl_set):
        ...
        filename = os.path.join(
            folder_path, 'intersection_controller_set.json')
        if os.path.isfile(filename):
            with open(filename, 'r') as f:
                saved_info = json.load(f)
        else:
            Logger.log_warning(
                'There is no intersection_controller_set.json. An empty IntersectionControllerSet instance will be returned.')
            return ic_set
    ...
    def load_lane_marking(folder_path):
        ...
        if os.path.isfile(filename):
            ...
            Logger.log_warning(
                'There is no lane_marking_set.json. An empty SignalSet instance will be returned.')
            return lane_set, node_set
    ...
    def create_instance_from_json(folder_path):
        ...
        global_info, node_set, link_set, junction_set = MGeoPlannerMap.load_node_and_link(
            folder_path)

        lane_marking_set, lane_node_set = MGeoPlannerMap.load_lane_marking(
            folder_path)
        ...
        if supported_class['synced_signal']:
            synced_light_set = MGeoPlannerMap.load_synced_traffic_light(
                folder_path, link_set, light_set)
        else:
            synced_light_set = None

        if supported_class['intersection_controller']:
            intersection_controller_set = MGeoPlannerMap.load_intersection_controller(
                folder_path, synced_light_set)
        else:
        ...
        mgeo_planner_map.local_origin_in_global = np.array(
            global_info['local_origin_in_global'])
